...
class BenchmarkRequest(BaseModel):
    ...
    benchmark_type: str = "full"  # "ttfa", "streaming", "full", "model-optimization"
    include_voice_tests: bool = True
    include_speed_tests: bool = True
    save_results: bool = True
    custom_text: Optional[str] = None
    enable_comparison: bool = False  # For model optimization benchmarks
    trials: int = 5  # Number of trials for model optimization
...
async def get_benchmark_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "benchmark_types_available": [
            "ttfa",
            "streaming",
            "full",
            "model-optimization",
        ],
        "last_execution": _benchmark_status.get("last_results", {}).get(
            "execution_timestamp"
        )
        if _benchmark_status.get("last_results")
        else None,
    }
...
async def get_benchmark_report() -> Dict[str, str]:
    ...
    try:
        ...
        return {
            ...
            "benchmark_timestamp": _benchmark_status["last_results"].get(
                "execution_timestamp"
            ),
        }
...
async def run_quick_ttfa_test(text: str = "Quick TTFA test") -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "recommendation": _get_quick_recommendation(
                measurement.total_ttfa_ms, measurement.bottleneck
            ),
        }
    except Exception as e:
...
async def _execute_benchmark(
    ...
    try:
        ...
        elif benchmark_type == "model-optimization":
            await _execute_model_optimization_benchmark(
                custom_text or "Test text for model optimization comparison", 5, False
            )
        else:
        ...
        _benchmark_status.update(
            {"running": False, "current_test": f"Failed: {str(e)}", "progress": 0.0}
        )
    finally:
...
async def _execute_model_optimization_benchmark(
    text: str, trials: int = 5, enable_comparison: bool = False
):
    """Execute model optimization benchmark"""
    _benchmark_status["current_test"] = "Model Optimization Benchmark"
    _benchmark_status["progress"] = 0.1

    try:
        suite = ModelOptimizationBenchmarkSuite(server_url="http://localhost:8000")
        results = await suite.run_comprehensive_comparison(
            original_model_path=TTSConfig.MODEL_PATH,
            optimized_model_path=TTSConfig.OPTIMIZED_MODEL_PATH,
            test_texts=[text],
        ...
        logger.error(f"Model optimization benchmark failed: {e}")
        raise
...
def _get_optimization_recommendations(stats: Dict[str, Any]) -> List[str]:
    ...
    if ttfa_avg > 1200:
        recommendations.append(
            "Enable streaming optimization for better TTFA performance"
        )
    if success_rate < 0.8:
...
async def _execute_full_benchmark():
    ...
    # Note: Save functionality temporarily disabled until BenchmarkRunner is implemented
...
def _estimate_benchmark_duration(benchmark_type: str) -> float:
    ...
    estimates = {
        ...
        "model-optimization": 60.0,  # 1 minute for model optimization comparison
    }
...
@router.post("/model-optimization")
async def run_model_optimization_benchmark(
    request: BenchmarkRequest, background_tasks: BackgroundTasks
) -> Dict[str, Any]:
    """
    Run model optimization comparison benchmark.

    Compares original vs optimized ONNX models side-by-side,
    measuring TTFA, RTF, memory usage, and generating comparison reports.
    """
    if not TTSConfig.ENABLE_MODEL_OPTIMIZATION_TESTING:
        raise HTTPException(
            status_code=403,
            detail="Model optimization testing is disabled. Set KOKORO_ENABLE_MODEL_OPTIMIZATION_TESTING=true",
        )

    if not os.path.exists(TTSConfig.OPTIMIZED_MODEL_PATH):
        raise HTTPException(
            status_code=404,
            detail=f"Optimized model not found: {TTSConfig.OPTIMIZED_MODEL_PATH}",
        )
    ...
    # Start benchmark in background
    background_tasks.add_task(
        _execute_model_optimization_benchmark,
        request.custom_text or "Test text for model optimization comparison",
        request.trials,
        request.enable_comparison,
    )

    return {
        "message": "Model optimization benchmark started",
        "benchmark_id": f"model_opt_{int(time.time())}",
        "estimated_duration_s": request.trials * 10,  # Rough estimate
        "original_model": TTSConfig.MODEL_PATH,
        "optimized_model": TTSConfig.OPTIMIZED_MODEL_PATH,
        "status_endpoint": "/benchmarks/status",
        "results_endpoint": "/benchmarks/results",
    }

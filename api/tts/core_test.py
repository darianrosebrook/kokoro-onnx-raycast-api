...
async def stream_tts_audio(
    ...
    try:
            ...
            try:
                ...
                if use_fast:
                    idx, audio_np, provider = await run_in_threadpool(
                        _fast_generate_audio_segment, i, segment_text, voice, speed, lang, request_id, no_cache
                    )
                else:
                    idx, audio_np, provider = await run_in_threadpool(
                        _generate_audio_with_fallback, i, segment_text, voice, speed, lang, request_id, no_cache
                    )
                ...
                # Track audio size variation for optimization (last segment)
                if i == len(segments) - 1:
        ...
        variation_handler.record_stream_health(
            ...
            stream_success=stream_success and processed_segments > 0,
        ...
        logger.info(f"[{request_id}] Stats: segments={processed_segments}/{len(segments)}, "

...
def generate_report(results: Dict[str, Any], output_file: Optional[str] = None) -> str:
    ...
    report.append("")
    report.append("| Model | Size (MB) | Provider | Avg (ms) | Min (ms) | Max (ms) | P50 (ms) | P95 (ms) | Std (ms) |")
    report.append("|-------|-----------|----------|----------|----------|----------|----------|----------|----------|")
    
    for model_name, data in results.items():
        if 'error' in data:
            report.append(f"| {model_name} | {data['size_mb']:.1f} | ERROR | - | - | - | - | - | - |")
        else:
            report.append(f"| {model_name} | {data['size_mb']:.1f} | {data['provider']} | {data['avg']:.2f} | {data['min']:.2f} | {data['max']:.2f} | {data['p50']:.2f} | {data['p95']:.2f} | {data['std']:.2f} |")
    ...
    if valid_results:
        ...
        report.append(f"**Fastest Model:** {best_model[0]} ({best_model[1]['avg']:.2f}ms avg)")
        ...
        for model_name, data in valid_results.items():
            ...
            report.append(f"- **{model_name}**: {data['size_mb']:.1f}MB ({size_reduction:+.1f}% size) → {data['avg']:.2f}ms avg")
    ...
    for provider in providers_used:
        provider_models = [k for k, v in valid_results.items() if v['provider'] == provider]
    ...
    if valid_results:
        ...
        report.append(f"1. **For maximum speed**: Use {fastest[0]} ({fastest[1]['avg']:.2f}ms)")
        report.append(f"2. **For minimum size**: Use {smallest[0]} ({smallest[1]['size_mb']:.1f}MB)")
        ...
        if len(valid_results) > 1:
            ...
            for model_name, data in valid_results.items():
                ...
                time_score = data['avg'] / fastest[1]['avg']  # Normalize to fastest time
                efficiency_score = (size_score + time_score) / 2
                data['efficiency_score'] = efficiency_score
            
            best_balance = min(valid_results.items(), key=lambda x: x[1]['efficiency_score'])
            report.append(f"3. **Best balance**: Use {best_balance[0]} (efficiency score: {best_balance[1]['efficiency_score']:.3f})")
